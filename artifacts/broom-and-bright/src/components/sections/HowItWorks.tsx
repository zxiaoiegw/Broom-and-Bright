            ...
            {steps.map((step, index) => (
              <div key={index} className="text-center relative">
                <div className="w-24 h-24 mx-auto bg-white rounded-full shadow-lg flex items-center justify-center mb-6 relative border-4 border-slate-50">
                  <div className="absolute -top-3 -right-3 w-8 h-8 bg-[#3fae74] text-white rounded-full flex items-center justify-center font-bold text-sm border-4 border-slate-50 shadow-sm">
                    {index + 1}
                  </div>
                  <step.icon className="w-10 h-10 text-[#3fae74]" />
                </div>
